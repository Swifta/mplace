   ...
   $("input.required").each(function(){
	...
	$('.cart_button').click(function() {
		...
		$('#submit_acc').click(function() {
			...
			/*
				Need to validate that form value.
				#Live
			*/
			var url = Path+'users/club_registration_logged_in_user/'+nuban; 
		        ...
		        success:function(response){
					...
					*/
					...
					for(i; i < classes.length; i++){
						...
						var class = $(classes[i]);
						var name = $(class.children()[0]).text();
						var no = $(class.children()[1]).text();
