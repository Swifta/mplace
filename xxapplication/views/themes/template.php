...
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0" />
        ...
        <link rel="shortcut icon" href="<?php echo PATH . '../../../application - Copy/views/themes/resize.php'; ?>?src=<?php echo PATH; ?>themes/<?php echo THEME_NAME; ?>/images/favicon.png&w=<?php echo FAVICON_WIDTH; ?>&h=<?php echo FAVICON_HEIGHT; ?>" type="image/x-icon" />		
        <?php
        if ($this->city_id) {
            foreach ($this->all_city_list as $CX) {
                if ($this->city_id == $CX->city_id) {
                    ?>
                    <link rel="alternate" type="application/atom+xml" title="<?php echo ucfirst($CX->city_name); ?> deals" href="<?php echo PATH . '../../../application - Copy/views/themes/deals/rss' . $this->city_id . '/' . $CX->city_url; ?>"  />
                    <?php
                }
            }
        }
