...
class Authorize_Controller extends Layout_Controller
{
	...
	public function __construct()
	{
		...
		foreach($this->generalSettings as $s){
	        ...
	        define("AUTHORIZENET_API_LOGIN_ID", "6592sDqET"); //$s->authorizenet_api_id
	        define("AUTHORIZENET_TRANSACTION_KEY", "8eJxt6658gZy5ZMT"); //$s->authorizenet_transaction_key
	         ...
	         foreach($_SESSION as $key=>$value)
                {
                    ...
                    $product_size = "1";
						...
						//var_dump($this->product_size_details);
						//echo $this->product_size_details->current()->quantity;
						//exit(0);
                    ...
                    if ($response->approved) {
                        ...
                        foreach($_SESSION as $key=>$value)
                        {
                            if($value && (is_string($value)) && ($key=='product_cart_id'.$value)){
                                ...
                                $transaction = $this->authorize->insert_product_transaction_details($responseheader, $deal_amount,$deal_id, $transaction_id,$country_code, $currencyCode,$post->firstName, $post->firstName, $referral_amount, $qty, 4, $captured, $purchase_qty, $friend_gift, $friendName, $friendEmail,$merchant_id,arr::to_object($this->userPost), $product_size,$product_color,$tax_amount,$shipping_amount, $shipping_methods);
				        ...
				        if ($response->approved) {
							...
							common::message(-1, "Checkout can not be approved at the moment, please try again.");
                   			url::redirect(PATH."cart.html");
						}
		...
		if($captured == 0){
			...
			foreach($captured_mail_list as $mail){
                ...
                $friend_name = $friend_details->current()->friend_name;
			     $this->result_mail = arr::to_object(array("deal_title" => $mail->deal_title, "item_qty" => $mail->quantity ,"total" => $mail->amount + $mail->referral_amount,"ref_amount"=> $mail->referral_amount, "amount"=> $mail->amount ,"friend_name" => $friend_name,"user_name" => $mail->firstname,"value" =>$mail->deal_value,"merchant_id" =>$mail->merchant_id));
