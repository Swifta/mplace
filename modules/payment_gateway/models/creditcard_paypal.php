...
class Creditcard_paypal_Model extends Model
{
	...
	public function insert_transaction_details($R = "", $deal_id = "", $country_code = "", $firstName = "", $lastName = "", $ref_amount = 0, $qty = 1, $type = 1, $captured = 0, $purchase_qty = "" ,$post = "",$merchant_id = "",$friend_gift = "",$friendName = "",$friendEmail = "",$product_size = "",$product_color = "",$tax_amount = "" ,$shipping_amount = "", $deal_amount = "", $shipping_methods = "")
	{
		...
		$result = $this->db->insert("transaction",array("user_id" => $this->UserID , "product_id" => $deal_id, "country_code" => $country_code, "currency_code" => CURRENCY_CODE, "transaction_date" => strtotime($R->TIMESTAMP), "correlation_id" => $R->CORRELATIONID, "acknowledgement" => "Success", "firstname" => $this->UserName, "lastname" => $this->UserName, "transaction_id" => $R->TRANSACTIONID, "order_date" => time(), "amount" => $deal_amount , "referral_amount" => $ref_amount, "payment_status" => "Completed", "quantity" => $qty, "type" => $type, "captured" => $captured,'deal_merchant_commission' => $commission_amount,"friend_gift_status" => $friend_gift,"product_size" => $product_size, "product_color"=>$product_color,"shipping_amount"=>$shipping_amount, "tax_amount"=>$tax_amount, "shipping_methods"=>$shipping_methods, "aramex_currencycode"=>$aramex_currencycode, "aramex_value"=>$aramex_value));
	...
	public function insert_paypal_transaction_details($R = "", $payer="", $type = "",$deal_id = "", $referral_amount = "", $qty = "", $purchase_qty = "", $captured = "",$paymentType = "",$deal_amount = "",$merchant_id = "",$friend_name= "",$friend_email = "", $friend_gift_status = "",$adderss1= "",$adderss2 = "", $state = "",$city= "",$country = "",$product_size = "",$product_color = "", $shipping_name = "", $postal_code= "",$phone = "",$tax_amount = "",$shipping_amount = "", $shipping_methods = "")
	{
		...
		$result = $this->db->insert("transaction",array("user_id" => $this->UserID , "product_id" => $deal_id, "payer_id" => $payer->PAYERID , "payer_status" => $payer->PAYERSTATUS, "paypal_email" => $payer->EMAIL ,"country_code" => COUNTRY_CODE, "currency_code" => CURRENCY_CODE, "transaction_date" => strtotime($R->TIMESTAMP), "correlation_id" => $R->CORRELATIONID, "transaction_id" => $R->TRANSACTIONID, "transaction_type" =>  $R->TRANSACTIONTYPE, "payment_type" => $paymentType, "acknowledgement" => "Success", "firstname" => $this->UserName, "lastname" => $this->UserName, "order_date" => time(), "amount" => $deal_amount, "payment_status" => "Completed", "pending_reason" => $R->PENDINGREASON, "reason_code" => $R->REASONCODE , "referral_amount" => $referral_amount, "quantity" => $qty, "type" => $type, "captured" => $captured,'deal_merchant_commission' => $commission_amount,"friend_gift_status" => $friend_gift_status,"product_size" => $product_size, "product_color"=>$product_color,"shipping_amount"=>$shipping_amount, "tax_amount"=>$tax_amount, "shipping_methods"=>$shipping_methods, "aramex_currencycode"=>$aramex_currencycode, "aramex_value"=>$aramex_value));
		$trans_ID = $result->insert_id();
