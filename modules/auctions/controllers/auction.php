...
class Auction_Controller extends Layout_Controller
{
	...
	public function search_list( $page = "")
	{
		...
		$this->template->title = rtrim($this->Lang['AUCTION'].'/'.$search." | ".SITENAME);
		$this->title_display = rtrim($this->Lang['AUCTION'].'/'.$search);
		$this->template->content = new View("themes/".THEME_NAME."/auction/auction");
	...
	public function details_auction($deal_key = "", $url_title = "",$type = "")
	{
	        $this->is_auction = 1;
	        $this->is_details = 1;
	...
	public function success_bidding()
	{
		...
		if($_POST){
				...
				if($status==0)
				{
					...
					url::redirect(PATH."auction/".$this->deal_key."/".$this->url_title.".html");
				}
			} else {
