...
class Auction_Model extends Model
{
	...
	if(CITY_SETTING){ 
	$query = "select auction.deal_id,auction.deal_key,auction.deal_title,auction.url_title,auction.deal_value,auction.deal_price, category.category_url,product_value,auction.enddate from auction  join stores on stores.store_id=auction.shop_id join category on category.category_id=auction.category_id where enddate > ".time()."  and deal_status = 1  ".$this->auction_club_condition." and category.category_status = 1 and stores.city_id = '$this->city_id' and  store_status = 1 order by auction.view_count DESC limit 3 ";

	$result = $this->db->query($query);
	} else {
	$query = "select auction.deal_id,auction.deal_key,auction.deal_title,auction.url_title,auction.deal_value,auction.deal_price, category.category_url,product_value,auction.enddate from auction  join stores on stores.store_id=auction.shop_id join category on category.category_id=auction.category_id where enddate > ".time()."  and deal_status = 1  ".$this->auction_club_condition." and category.category_status = 1 and  store_status = 1 order by auction.view_count DESC limit 3 ";
	...
	public function  get_search_deals_list($search = "",  $offset = "", $record = "",$maincatid="")
	{
		...
		$query = "select auction.deal_id,auction.deal_key,auction.deal_title,auction.url_title,auction.deal_value,auction.deal_price, category.category_url,product_value,auction.enddate from auction  join stores on stores.store_id=auction.shop_id join category on category.category_id=auction.category_id where $conditions order by deal_id DESC limit $offset,$record";
		$result = $this->db->query($query);
	        return $result;
	}
	...
	public function  get_hot_deals_view()
	{
	        if(CITY_SETTING){ 
		 $conditions = "enddate > ".time()." and deal_status = 1  ".$this->auction_club_condition." and category.category_status = 1 and  store_status = 1 and auction_status = 0 ";
		$query = "select * from auction  join stores on stores.store_id=auction.shop_id join category on category.category_id=auction.category_id where $conditions and deal_feature = 1 and stores.city_id = '$this->city_id'  ORDER BY RAND() limit 4";
		$result = $this->db->query($query);
		} else {
		$conditions = "enddate > ".time()." and deal_status = 1  ".$this->auction_club_condition." and category.category_status = 1 and  store_status = 1 and auction_status = 0 ";
		$query = "select auction.deal_id,auction.deal_key,auction.deal_title,auction.url_title,auction.deal_value,auction.deal_price, category.category_url,product_value,auction.enddate from auction  join stores on stores.store_id=auction.shop_id join category on category.category_id=auction.category_id where $conditions and deal_feature = 1 ORDER BY RAND() limit 4";
		$result = $this->db->query($query);
		}
	        return $result;
	}
	...
	public function get_deals_details($deal_key = "", $url_title = "",$type = "")
	{
			...
			$condition = array("url_title" => $url_title, "deal_key" => $deal_key,"category.category_status" => 1, "store_status" => 1);
				...
				$country = "";
			       ...
			       $condition = array("url_title" => $url_title, "deal_key" => $deal_key, "auction.deal_status" => 1, "category.category_status" => 1, "store_status" => 1,);//@Live

				   if($this->auction_club_condition_arr)
				   		$condition = array("url_title" => $url_title, "deal_key" => $deal_key, "auction.deal_status" => 1, "category.category_status" => 1, "store_status" => 1, "auction.for_store_cred" => "0");
	...
	public function get_payment_auction_details($deal_id = "")
	{
		...
		$n_condition = array("deal_id" =>$deal_id,"deal_status" =>1,"auction_status" =>1);
		if($this->auction_club_condition_arr)
			$n_condition = array("deal_id" =>$deal_id,"deal_status" =>1,"auction_status" =>1, "auction.for_store_cred" => 0);
			
		
		
		$result = $this->db->from("auction")->where($n_condition)->get();//@Live
		return $result;
	}
	...
	{
		$condition = array("deal_id <>" => $deal_id,"auction.sec_category_id" =>$category_id, "enddate >" => time(),"deal_status" => 1, "category.category_status" => 1, "store_status" => 1,"auction_status" => 0);//@Live
		
		if($this->auction_club_condition_arr)
			$condition = array("deal_id <>" => $deal_id,"auction.sec_category_id" =>$category_id, "enddate >" => time(),"deal_status" => 1, "category.category_status" => 1, "store_status" => 1,"auction_status" => 0, "auction.for_store_cred" => 0);
		
	 if(CITY_SETTING){
		 
		 $condition = array("deal_id <>" => $deal_id,"auction.sec_category_id" =>$category_id, "stores.city_id" => $this->city_id, "enddate >" => time(),"deal_status" => 1, "category.category_status" => 1, "store_status" => 1,"city.city_status" => 1); //@Live
		 
		 if($this->auction_club_condition_arr)
		 	$condition = array("deal_id <>" => $deal_id,"auction.sec_category_id" =>$category_id, "stores.city_id" => $this->city_id, "enddate >" => time(),"deal_status" => 1, "category.category_status" => 1, "store_status" => 1,"auction.for_store_cred" => 0,"city.city_status" => 1);
		}
	...
	public function get_deals_list_map()
	{
				...
				if(auction_club_condition_arr)
					$contition = array("stores.city_id" => $this->city_id ,"enddate >" => time(), "deal_status" => 1,"city_status" => 1, "auction.for_store_cred" => 0);
		        $result = $this->db->from("deals")
                                                ->join("stores","stores.store_id","deals.shop_id")
                                                ->join("city","city.city_id","stores.city_id")
                                                ->join("category","category.category_id","deals.category_id")
                                                ->where($contition) //@Live
	...
	public function  get_ajax_auctions_list($price="",$main_cat="",$sub_cat="",$sec_cat="",$third_cat="",$price_text="",$type="")
	{
		...
		if(CITY_SETTING){
			$query = "select auction.deal_id,auction.deal_key,auction.deal_title,auction.url_title,auction.deal_value,auction.deal_price, category.category_url,auction.enddate from auction join stores on stores.store_id=auction.shop_id $join where enddate > ".time()." and deal_status = 1  ".$this->auction_club_condition." and auction_status = 0 and category.category_status = 1 and  store_status = 1 and stores.city_id = '$this->city_id'  $conditions group by auction.deal_id order by auction.deal_id DESC $pagin";
		$result = $this->db->query($query);

		} else {
			$query = "select auction.deal_id,auction.deal_key,auction.deal_title,auction.url_title,auction.deal_value,auction.deal_price, category.category_url,auction.enddate from auction  join stores on stores.store_id=auction.shop_id $join where enddate > ".time()." and auction_status = 0 and deal_status = 1  ".$this->auction_club_condition." and category.category_status = 1 and  store_status = 1 $conditions group by auction.deal_id order by auction.deal_id DESC $pagin";
			$result = $this->db->query($query);
