...
class Home_Model extends Model
{
	...
	public function products_details($cityid="", $category_id = "")
	{
		if(CITY_SETTING){
			...
			$result = $this->db->query("select product.deal_id,product.deal_key,product.deal_title,product.url_title,product.deal_value,product.deal_price, category.category_url,deal_percentage from product  join stores on stores.store_id=product.shop_id join category on category.category_id=product.category_id  where deal_status = 1 and  store_status = 1 and product.category_id = '$category_id' ".$this->product_club_condition." and stores.city_id = '$cityid'  and purchase_count < user_limit_quantity order by deal_id DESC limit 4 ");
		} else {
			$result = $this->db->query("select product.deal_id,product.deal_key,product.deal_title,product.url_title,product.deal_value,product.deal_price, category.category_url,deal_percentage from product  join stores on stores.store_id=product.shop_id join category on category.category_id=product.category_id  where deal_status = 1 and  store_status = 1  and product.category_id = '$category_id' ".$this->product_club_condition." and purchase_count < user_limit_quantity order by deal_id DESC limit 4 ");
	...
	public function auction_details($cityid="")
	{
		if(CITY_SETTING) {
					...
					if($this->auction_club_condition_arr)
						$n_condition = array("deal_status" => 1,"enddate >" => time(),"startdate <" => time(),"stores.city_id" => $cityid,"category.category_status" => 1, "store_status" => 1,"auction_status" => 0, "auction.for_store_cred" => 0);
	        ...
	        return $result;
			 ...
			 if($this->auction_club_condition_arr)
			 $n_condition = array("deal_status" => 1,"enddate >" => time(),"startdate <" => time(),"category.category_status" => 1, "store_status" => 1,"auction_status" => 0,  "auction.for_store_cred" => 0);
	...
	{               
	        	...
	        	$contition = array("stores.city_id" => $this->city_id ,"deal_status" => 1,"enddate >" => time(),"startdate <" => time(),"city_status" => 1,"auction_status" => 0);//@Live
		        $result = $this->db->from("auction")
		                ->join("stores","stores.store_id","auction.shop_id")
						...
						$contition = array("deal_status" => 1,"enddate >" => time(),"startdate <" => time(),"auction_status" => 0);//@Live
						$result = $this->db->from("auction")
								->join("stores","stores.store_id","auction.shop_id")
                                ->join("category","category.category_id","auction.category_id")
                                ->where($contition)
	...
	{ 
			...
			/*
				This query breaks on adding deal_club_condition
			*/
	...
	public function get_deals_country_lists($country_id = "",  $category = "", $source = "")
	{
		if($category || $category == "others"){
			...
			$contition = array("deals.country_id" => $country_id ,"enddate >" => time(), "deal_status" => 1,  "city_status" => 1, "deals.category_id" =>  $category, "deals.for_store_cred" => 0);//@Live
			...
			if($this->deal_club_condition_arr)
				$contition = array("deals.country_id" => $country_id ,"enddate >" => time(), "deal_status" => 1, "city_status" => 1, "deal.for_store_cred" => 0);//@Live
		}
		...
		if($source){
			...
			if($this->deal_club_condition_arr)
				$contition = array("deals.country_id" => $country_id ,"enddate >" => time(), "deal_status" => 1, "city_status" => 1, "deals.for_store_cred" => 0);//@Live
		}
	...
	public function get_deals_lists_by_filter($city_id = "" , $sources = "" , $category_tags= "")
	{
		...
		$n_condtion = array("deals.city_id" => $city_id ,"enddate >" => time(), "deal_status" => 1, "city_status" => 1);
		if($this->deal_club_condition_arr)
			$n_condtion = array("deals.city_id" => $city_id ,"enddate >" => time(), "deal_status" => 1, "city_status" => 1, "deals.for_store_cred" => 0);
	...
	public function get_rss_deals_lists_country($country_id = "")
	{
		...
		if($this->auction_club_condition_arr)
			$n_condtion = array("deals.country_id" => $country_id ,"enddate >" => time(),"startdate <" => time(), "deal_status" => 1, "country_status" => 1, "deals.for_store_cred" => 0);
		
		$result = $this->db->from("deals")
			->join("country","country.country_id","deals.country_id")
			->where($n_condtion)
			->orderby("deals.enddate", "ASC")//@Live
			->get();
		return $result;
	}
	...
	{
		$n_condtion = array("deal_id" => $deal_id, "deal_key" => $deal_key, "deal_status" => 1);
		if($this->deal_club_condition_arr)
			$n_condtion = array("deal_id" => $deal_id, "deal_key" => $deal_key, "deal_status" => 1, "deals.for_store_cred" => 0);
			
		$result = $this->db->select("url_title")->from("deals")->where($n_condition)->get();//@Live
		return $result;
	}
...

	{
	        $result_category = $this->db->select("category_id")->from("category")
				...
				if($this->deal_club_condition_arr)
		 			$conditions = array("deals.deal_type" => 1, "deal_status" => 1,"deals.category_id" => $category_di_value,"stores.city_id" => $this->city_id, "deals.for_store_cred" => 0); 
		 
		}  //@Live
		else{
		 $conditions= array("deals.deal_type" => 2, "deal_status" => 1,"deals.category_id" => $category_di_value,"stores.city_id" => $this->city_id);
		 if($this->product_club_condition_arr || $this->auction_club_condition_arr )
...
/* GET AUCTION ALERT */
	...
	public function  get_search_deals_list($search = "", $category="", $cat_type="",$maincatid="")
	{ 
	        $conditions = " ";
	...
	public function  get_search_products_list($search = "", $category = "",$cat_type="",$maincatid="")
	{
	...
	public function  get_search_auction_list($search = "", $category="", $cat_type="",$maincatid="")
	{
		 $conditions = " ";
