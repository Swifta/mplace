...
class Welcome_Controller extends Layout_Controller
{
					...
					foreach($this->deals_list as $deal){
						if (($deal->maximum_deals_limit == $deal->purchase_count) || ($deal->maximum_deals_limit < $deal->purchase_count) || ($deal->enddate < time())) { } else { 
							if($deal->shop_id == $store_id){
								 ...
								 $dealUrl = PATH."deals/".$deal->deal_key."/".$deal->url_title.".html";
					...
					foreach($this->deals_list as $deal){
						if($deal->shop_id == $store_id){
							  ...
							  if(!file_exists(DOCROOT.$imgpath)){
							   ...
							   $category_name=$deal->category_name;		
							  $imgpath = PATH.$imgpath;
							 $dealUrl = PATH."product/".$deal->deal_key."/".$deal->url_title.".html";
				...
				$this->all_payment_list = $this->home->payment_list();
					foreach($this->deals_list as $deal){
						...
						if($deal->shop_id == $store_id){
							  ...
							  $imgpath = PATH.$imgpath;
							 $dealUrl = PATH."auction/".$deal->deal_key."/".$deal->url_title.".html";
								 $q=0; 
	...
	public function ajax_search($search = "",$type = "")
	{ 
		$this->type = $type;
		$this->search = $search;
		...
		if($this->type == 1 || $this->type == 2 || $this->type == 3 || $this->type == 4 || $this->type == 5 || $this->type == 0) { 
			$this->deals_details = $this->home->get_search_deals_list($search,"","");
			$this->products_details = $this->home->get_search_products_list($search,"","");
			$this->auction_details = $this->home->get_search_auction_list($search,"","");
		}
