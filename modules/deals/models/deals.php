...
class Deals_Model extends Model
{
	...
	public function get_deals_list($cat_type,$category = "", $offset = "", $record = "")
	{
		...
		if($category && $cat_type=='main'){
			...
			$result = $this->db->query("select deals.deal_id,deals.deal_key,deals.deal_title,deals.url_title,deals.deal_value,deals.deal_price, category.category_url, deals.maximum_deals_limit, deals.purchase_count,deals.enddate,deals.deal_percentage  from deals  join stores on stores.store_id=deals.shop_id join category on category.category_id = $join where $conditions group by deal_id order by deal_id DESC limit $offset,$record ");
			//print_r($result);
		return $result;
	}
	...
	public function  get_search_deals_list($search = "",  $offset = "", $record = "",$maincatid="")
	{
		...
		$query = "select deals.deal_id,deals.deal_key,deals.deal_title,deals.url_title,deals.deal_value,deals.deal_price, category.category_url,deals.maximum_deals_limit, deals.purchase_count,deals.enddate,deals.deal_percentage  from deals  join stores on stores.store_id=deals.shop_id join category on category.category_id=deals.category_id where $conditions order by deal_id DESC limit $offset,$record";
		$result = $this->db->query($query);
	        return $result;
	}
	...
	if(CITY_SETTING){ 
	$query = "select deals.deal_id,deals.deal_key,deals.deal_title,deals.url_title,deals.deal_value,deals.deal_price, category.category_url, deals.maximum_deals_limit, deals.purchase_count,deals.enddate,deals.deal_percentage  from deals  join stores on stores.store_id=deals.shop_id join category on category.category_id=deals.category_id where enddate > ".time()." and purchase_count < maximum_deals_limit and deal_status = 1  ".$this->deal_club_condition." and category.category_status = 1 and stores.city_id = '$this->city_id' and  store_status = 1 order by deals.view_count DESC limit 3 ";
				...
				$country = "";
				   ...
				   $condition = array("url_title" => $url_title, "deal_key" => $deal_key, "deal_status" => 1, "category.category_status" => 1, "store_status" => 1);
				    ...
				    if($this->deal_club_condition_arr){
						$condition = array("url_title" => $url_title, "deal_key" => $deal_key, "deal_status" => 1, "deals.for_store_cred" => 0, "category.category_status" => 1, "store_status" => 1);
...
/*	        $result = $this->db->select("*","stores.phone_number as phone")->from("deals")
	...
	{
		$n_conditions = array("url_title" => $url_title, "deal_key" => $deal_key,"deal_status" => 1);;
		if($this->deal_club_condition_arr){
			$n_conditions = array("url_title" => $url_title, "deal_key" => $deal_key,"deal_status" => 1, "deals.for_store_cred" => 0);
		}
