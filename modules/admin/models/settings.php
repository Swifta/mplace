...
class Settings_Model extends Model
{
	...
	public function update_payment_settings($post = "")
	{
		...
		$status = $this->db->update("settings",array("min_fund_request" => $post->minfund, "max_fund_request" => $post->maxfund, "currency_symbol" => trim($post->currency_symbol) , "currency_code" => trim($post->currency_code),"country_code" => trim($post->country_code),"referral_amount" => $post->referral_amount,"paypal_payment_mode" => $post->paypal_payment_mode,"paypal_account_id" => $post->Paypal_Account,"paypal_api_password" => $post->Paypal_API_Password,"paypal_api_signature" => $post->Paypal_API_Signature,"authorizenet_transaction_key" => $post->authorizenet_transaction_key,"authorizenet_api_id" => $post->authorizenet_api_id,"tax_percentage" => $post->tax_percentage,"flat_shipping" => $post->flat_shipping,"auction_extend_day" =>$post->auction_extend_day,"auction_alert_day" => $post->auction_alert_day), array("id" => 1)); 
		return 1;
	}
