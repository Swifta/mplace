...
class Admin_products_Model extends Model
{
	...
	public function add_product($post = "", $deal_key = "",$adminid = "",$size_quantity = "")
	{
		...
		$result = $this->db->insert("product", array("deal_title" => $post->title, "url_title" => url::title($post->title), "deal_key" => $deal_key, "deal_description" => $post->description,"delivery_period" => $post->delivery_days,"category_id" => $post->category,"sub_category_id" =>  $post->sub_category,"sec_category_id" =>  $post->sec_category,"third_category_id" => $post->third_category, "deal_type"=> $post->deal_type, "deal_price" => $post->price,"deal_value" => $post->deal_value,"deal_savings" => $savings,"meta_keywords" => $post->meta_keywords , "meta_description" =>  $post->meta_description,"deal_percentage" => ($savings/$post->price)*100,"merchant_id"=>$post->users,"shop_id"=>$post->stores,"created_date" => time(),"created_by"=>$adminid,"color" => $post->color_val,"size" => $post->size_val,"user_limit_quantity"=>$quantity,"deal_status" =>0,"shipping_amount"=>$shipping_amount,"shipping"=>$post->shipping,"attribute"=>$atr_option,"Including_tax" =>$inc_tax,"weight" => $weight,"height" => $height,"length" => $length,"width" => $width ));
	...
	public function get_all_products_list($type = "",$offset = "", $record = "", $city = "", $name = "",$sort_type = "",$param = "",$limit="",$today="", $startdate = "", $enddate = "")
	{
		...
		if($_GET){  
		        if($type != "1")
		        {
		                $conditions = "purchase_count < user_limit_quantity";
		        } else {
		                $conditions = "purchase_count >= user_limit_quantity ";
		        }
