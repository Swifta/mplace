...
class Admin_Model extends Model
{
	...
	{
		$result = $this->db->from("users")->where(array("email" => $email, "password" => md5($password), "user_type" => 1))->limit(1)->get();
		if(count($result) == 1){
			...
			$emailCount = $this->db->count_records("users", array("email" => $email, "user_type" => 1));
			if($emailCount == 0){
				return 8;
			}
			return 0;
	...
	{
		$result = $this->db->from("country")->orderby("country_url", "ASC")->get();
		return $result;
	}
	...
	{
		$result = $this->db->delete("country",array("country_url" => $country ));
		return count($result);
	}
	...
	public function add_city($country = "", $city = "",$post = "")
	{
		...
		$result = $this->db->count_records("city", array("country_id" => $country , "city_url" => url::title($city)));
		if($result == 0){
	...
	{
		$result = $this->db->delete("city",array("country_id" => $country, "city_url" => $city));
		return count($result);
	}
	...
	public function edit_category($category = "", $cat_status = "", $cat_id = "", $cat_url = "",$deal = "",$product = "",$auction = "")
	{
		if(url::title($category) != $cat_url){
			$result = $this->db->count_records("category", array("category_name" => url::title($category)));
			if($result > 0){
				return 0;
			}
		}
	...
	{
		$result = $this->db->delete("category",array("category_id" => $cat_id, "category_url" => $cat_url));
		return count($result);
	}
	...
	public function coupon_code_validate($code="")
	{
			...
			$conditions="";
              		if($code || $code=='0'){
                        ...
                        $query = "select deals.*,transaction_mapping.coupon_code,transaction_mapping.coupon_code_status,transaction.type,transaction.id as trans_id,transaction.amount,transaction.referral_amount,transaction.quantity from deals join transaction on transaction.deal_id=deals.deal_id  join transaction_mapping on transaction_mapping.transaction_id=transaction.id where $conditions and expirydate > $time limit 1 ";
	...
	/** GET USERS DATA  **/
        ...
        public function get_subscriber_list($offset = "", $record = "",  $category= "",$email="")
        {
			 ...
			 if($category){
		       ...
		       // $contitions .= ' AND city.city_name like "%'.mysql_escape_string($city).'%"';
                       ...
                       $result = $this->db->query("select *,email_subscribe.category_id as category_ids from email_subscribe  join category on category.category_id=email_subscribe.category_id    $contitions  order by subscribe_id DESC limit $offset, $record");
        ...
        public function get_subscriber_count($category = "",$email="")
        {
		       ...
		       // $contitions .= ' AND city.city_name like "%'.mysql_escape_string($city).'%"';
                       ...
                       $result = $this->db->query("select * from email_subscribe  join category on category.category_id=email_subscribe.category_id   $contitions  order by email_subscribe.subscribe_id DESC ");
