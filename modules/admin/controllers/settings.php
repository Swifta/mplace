	...
	{ 
		...
		if($_POST){ 
			...
			if($cash_on_delivery == ""){ $cash_on_delivery =0; }
				if(($paypal=="") && ($credit_card =="") && ($authorize =="") && ($cash_on_delivery =="")){
					common::message(-1, $this->Lang["PAY_SET_ERR1"]);
					url::redirect(PATH."admin/module-settings.html");
				}
				if(($paypal=="") && ($credit_card =="") && ($authorize =="")){
			...
			$status = $this->settings->update_module($deal,$product,$blog,$auction,$paypal,$credit_card,$authorize,$cash_on_delivery,$free_shipping,$flat_shipping,$per_product,$per_quantity,$aramex,$near_me_map,$store_list,$past_deal,$faq,$city,$cms,$newsletter);
			if($status==1){
