...
class Admin_products_Controller extends website_Controller
{
	...
	public function manage_product($type= "", $page = "")
	{
	    ...
	    if($type == 1){
	        ...
	        $this->count_products_all_record = $this->products->get_all_products_count($type, $this->input->get("city"), $this->input->get('name'),$this->type,$this->sort,$this->today,$this->startdate,$this->enddate);
		...
		$this->all_product_list = $this->products->get_all_products_list($type, $this->pagination->sql_offset, $this->pagination->items_per_page ,$this->input->get("city"), $this->input->get('name'),$this->type,$this->sort,0,$this->today,$this->startdate,$this->enddate);

		if($serch =='all'){ // for export all
			$this->all_product_list = $this->products->get_all_products_list($type, $this->pagination->sql_offset, $this->pagination->items_per_page ,$this->input->get("city"), $this->input->get('name'),$this->type,$this->sort,1,$this->today,$this->startdate,$this->enddate);
		}
	...
	public function edit_product($deal_id = "", $deal_key = "")
	{
	...
	$this->manage_product = "1";
			...
			$post = Validation::factory(array_merge($_POST,$_FILES))
				...
				if($post->validate()){
				    $size_quantity = $this->input->post("size_quantity");
					$status = $this->products->edit_product($deal_id, $deal_key, arr::to_object($this->userPost), $size_quantity);
