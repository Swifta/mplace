...
<?php  if(($this->uri->segment(2)=='view-auction')){	
		...
		<a class="fr" style="float:right;text-decoration: underline; cursor: pointer;position:relative; z-index:2;" href="<?php echo PATH.'merchant/view-auction/'.$this->auction_key.'/'.$this->auction_id.'.html?id=BID'  ?>" title="<?php echo $this->Lang['EXPO_BIDD_LI']; ?>"><img src="<?php echo PATH ?>images/csv.png" class="image" alt="<?php echo $this->Lang['EXPO_BIDD_LI']; ?>"/></a>
		<?php } } ?>
...
<?php if(count($this->transaction_auction_list) > 0){ ?>
        ...
        <table class="list_table fl clr mt20">
			...
			<th align="left" ><div class="arrow1"><a href="<?php echo $this->sort_url;?>param=title&sort=<?php if($this->input->get('sort')=='DESC'){ echo 'ASC'; }else{ echo 'DESC'; } ?>" title="<?php echo $this->Lang['SORT_B_DEAL_TIT']; ?>" ><?php echo $this->Lang['AUC_TIT']; ?></a></div></th>
			...
			<th align="left" ><div class="arrow3"><a href="<?php echo $this->sort_url;?>param=shipping_fee&sort=<?php if($this->input->get('sort')=='DESC'){ echo 'ASC'; }else{ echo 'DESC'; } ?>" title="<?php echo $this->Lang['SORY_B_SHIP_FEE']; ?>" ><?php echo $this->Lang['SHIP_FEE']; ?><?php echo '('.CURRENCY_SYMBOL.')';?></a></div></th>
			<?php } else { ?>	
			<th align="left"> <?php echo $this->Lang["USERS"]; ?> </th> 			
			<th align="left"> <?php echo $this->Lang['AUC_TIT']; ?> </th>
		   ...
		   <td align="left"><a href="<?php echo PATH.'merchant/view-auction/'.$u->deal_key.'/'.$u->deal_id.'.html';?>" title="<?php echo $u->deal_title; ?>"><?php echo htmlspecialchars($u->deal_title); ?></a></td>
		    <?php } else { ?>	
